...
\referToFigure{rewriting}, we see that for $q_1$ in the example above we apply
the rule $\proc{P-Times-Plus-R}$. This rewrites $q_1$ into

\[\underbrace{\p{c \lt c \lt x}}_{q_3} \lp \underbrace{\p{c \lt c \lt
y}}_{q_4},\]
...
\referToSection{chaining}. We specify only their left-associative parts because
...
bitwise operators in \referToFigure{rewriting}.
